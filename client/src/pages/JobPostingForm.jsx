...
import { jobAPI, recommendationsAPI } from '../services/api'
...
const JobPostingForm = () => {
  ...
  const voiceFields = [
    { field: 'title', label: 'Job Title', keywords: ['job title', 'position', 'role'] },
    { field: 'company.name', label: 'Company Name', keywords: ['company', 'organization', 'firm'] },
    { field: 'jobType', label: 'Job Type', keywords: ['job type', 'employment type', 'full time', 'part time', 'contract'] },
    { field: 'industry', label: 'Industry', keywords: ['industry', 'sector', 'field'] },
    { field: 'contactPerson.name', label: 'Contact Person Name', keywords: ['contact person', 'recruiter name', 'my name', 'contact name'] },
    { field: 'contactPerson.phone', label: 'Contact Number', keywords: ['phone', 'contact number', 'mobile', 'telephone'] }
  ]

  // State for detected answers
  ...
  // OpenAI-powered voice functionality methods
  const startVoiceEntry = async (e) => {
    ...
    console.log('Voice entry button clicked') // Debug log
    ...
    try {
      ...
      toast.success('🎤 Voice recording started! Speak all details in one go.')
  ...
  const processAudioWithOpenAI = async (audioBlob) => {
    try {
      ...
      console.log('Sending audio to OpenAI, size:', audioBlob.size)
      ...
      const response = await recommendationsAPI.processVoiceInput(audioBlob)

      if (!response.data) {
        throw new Error('No response data received')
      }

      const { transcription, extractedData } = response.data

      console.log('Transcription:', transcription)
      console.log('Extracted data:', extractedData)

      // Apply the extracted data to the form
      await applyExtractedDataToForm(extractedData, transcription)
      ...
      if (error.response?.status === 429) {
        ...
        toast.error('API authentication failed. Please check configuration.')
      } else if (error.name === 'NetworkError' || !navigator.onLine) {
        toast.error('Network error. Please check your internet connection.')
      } else {
        toast.error(`Voice processing failed: ${error.message || 'Unknown error'}`)
      }
    }
  }
              ...
              {isVoiceActive && (
                <div className="flex items-center space-x-2 sm:space-x-3 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
                  ...
                  <span className="font-medium hidden sm:inline">
                    {isRecording ? `Recording... ${recordingTime}s` : isProcessing ? 'Processing with AI...' : 'Initializing...'}
                  </span>
                  <span className="font-medium sm:hidden">
                    {isRecording ? '🎤' : isProcessing ? '🤖' : '⚡'}
                  </span>
                  <span className="text-xs bg-green-100 dark:bg-green-900 px-1 sm:px-2 py-1 rounded-full">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <div className={`relative transform transition-all duration-300 ${
                    isVoiceActive ? 'mic-icon-active scale-110' : 'group-hover:scale-110 group-hover:rotate-3'
                  }`}>
                    {isVoiceActive ? (
